...
# Crear factor con los meses ordenados
meses_ord <- factor(month.name, levels = month.name)

# Ordenar dataset por arrival_date_month
datos <- datos[order(factor(datos$arrival_date_month, levels = meses_ord)), ]
...
## Crear la gráfica
grafico<-ggplot(datos.hotel.resume, aes(x = arrival_date_month, y = stays_in_week_nights, fill = "Días de semana")) +
...
grafico <- grafico + scale_x_discrete(limits = meses_ord)
grafico
#----------------------------------------------------------------------------------------------------------
...
grafico2<-ggplot(datos.booking.filter.month.grouped, aes(x = arrival_date_month, y = count, fill = booking)) +
  geom_col(position = position_dodge()) +
  ...
  labs(title = "Comparación de reservas asignadas y realizadas por mes y hotel",
       ...
       fill = "Estado")
grafico2 <- grafico2 + scale_x_discrete(limits = meses_ord)
grafico2
